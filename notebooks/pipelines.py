...
def create_classification_pipeline_1(schema, iterations, created_columns, class_weights=None):
    ...
    # Update if additional engineered categorical features are created
    cat_features = categorical_features + ["arrival_season"] if "arrival_season" in created_columns else categorical_features
    ...
    class FeatureEngineer(BaseEstimator, TransformerMixin):
        ...
        def __init__(self, created_columns):
        ...
        def transform(self, X):
            ...
            if "total_stay" in self.created_columns:
                X["total_stay"] = X["stays_in_weekend_nights"] + X["stays_in_week_nights"]
    
            if "adr_per_person" in self.created_columns:
            ...
            if "arrival_season" in self.created_columns:
                X["arrival_season"] = X["arrival_date_week_number"].apply(week_to_season)
        
            return X

    feature_engineer = FeatureEngineer(created_columns)
    ...
    model = CatBoostClassifier(
        ...
        iterations=iterations,       # number of boosting rounds - lower for segments with less data
    ...
    steps = [
        ...
        ("feature_selector", FeatureSelector(required_features + feature_engineer.created_columns)),
        ("model", model),
    ]
