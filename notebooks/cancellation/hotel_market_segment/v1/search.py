...
name = f"cancellation_{args.segment}"
version = "v1"

try:
    feature_path = Path(f"data/features/cancellation/hotel_market_segment/{args.segment}_v1")
...
try:
    schema = pd.read_csv(Path(f"data/features/cancellation/hotel_market_segment/{args.segment}_v1/schema.csv"))
except Exception:
    logger.exception("Failed to load schema.")
    raise

try:
    pipeline = create_classification_pipeline_1(schema, 300, ["total_stay", "adr_per_person", "arrival_season"])
except Exception:
...
try:
    search_1 = search_best_params(pipeline, X_train, y_train, param_distributions_1, True, "broad")
...
try:
    ...
    search_2 = search_best_params(pipeline, X_train, y_train, param_distributions_2, True, "narrow")
...
feature_path = f"data/features/cancellation/hotel_market_segment/{args.segment}_v1"
features_version = "v1"
target = "is_canceled"
algorithm = "catboost"
