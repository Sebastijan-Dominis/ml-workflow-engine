...
def prepare_features(data: pd.DataFrame, config: TabularFeaturesConfig) -> pd.DataFrame:
    ...
    # include the entity key as well; fail if not present, as it's required for downstream steps and evaluation
    if entity_key not in data.columns:
        msg = f"Input data must contain '{entity_key}' column."
        logger.error(msg)
        raise DataError(msg)
    ...
    if missing_cols:
        ...
        logger.error(msg)
        raise DataError(msg)
    X = data[[entity_key] + INCLUDE].copy()
