    ...
    *,
    ...
    if transform_config.type == "log1p":
        ...
        try:
            ...
            msg = "Error applying log1p transformation to target."
        ...
        if (y_values < 0).any():
            ...
            msg = "Sqrt transformation requires non-negative target values. "
            f"Invalid values:\n{y_values[invalid_mask | np.isnan(y_values)]}"
            logger.error(msg)
        ...
        if (y_values <= 0).any():
            invalid_mask = y_values <= 0
            msg = "Box-Cox transformation requires strictly positive target values. "
            f"Invalid values:\n{y_values[invalid_mask | np.isnan(y_values)]}"
            logger.error(msg)
