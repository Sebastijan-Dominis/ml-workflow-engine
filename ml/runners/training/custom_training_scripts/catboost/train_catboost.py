...
from ml.runners.training.custom_training_scripts.catboost.train_catboost_model import train_catboost_model
...
def train_catboost(model_cfg: TrainModelConfig) -> tuple[CatBoostClassifier | CatBoostRegressor, Pipeline, list[dict], dict[str, float], str | None]:
...
logger = logging.getLogger(__name__)
    ...
    X_train, y_train, lineage_train = load_X_and_y(model_cfg, ["X_train", "y_train"], snapshot_selection=snapshot_selection)
    X_val, y_val, _ = load_X_and_y(model_cfg, ["X_val", "y_val"], snapshot_selection=snapshot_selection)
