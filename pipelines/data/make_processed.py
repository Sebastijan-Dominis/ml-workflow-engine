...
def main() -> int:
    ...
    data_dir = Path("data/processed") / args.dataset / args.version
    setup_logging(data_dir / "make_processed.log", level=log_level)

    try:
        config_raw = load_yaml(Path(f"configs/data/processed/{args.dataset}/{args.version}.yaml"))
        ...
        interim_metadata = load_json(Path(f"data/interim/{args.dataset}/{args.version}/metadata.json"))

        validate_dataset(data_path=Path(config.input.path), metadata=interim_metadata)
        ...
        metadata = prepare_metadata(df, config=config, start_time=start_time, dataset_path=dataset_path, owner=args.owner, memory_info=memory_info)
